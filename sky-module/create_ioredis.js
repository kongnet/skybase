...
const connect = function (rd) {
  ...
  redis.reconnectOnError = function (err) {
    ...
    $.err($.c.r('✘'), `-x- Redis [${$.c.y(`${rd.host} : ${rd.port}`)}] disconnect...`)
  ...
  redis.on('connect', function () {
    ...
    redis.dbsize().then(function (_r) {
      _r = _r || 0
      $.log($.c.g('✔'), `Redis [${$.c.y(`${rd.host} : ${rd.port}`)}] db ${$.c.y(rd.db)} [${$.c.y(_r)}] Objects`)
  ...
  redis.on('error', function (e) {
    $.err($.c.r('✘'), `-x- Redis [${$.c.y(`${rd.host} : ${rd.port}`)}] ${e.toString()}`)
  })
