...
const { routerDir, rootDir, apiDir, logger, serviceDir } = require('../../skyconfig')
...
const limitReqAction = (action, apiPath) => {
  return async function (ctx, next) {
    ...
    const { feqLimit, name, desc, limit = {} } = api[apiPath] || {}

    apiStat[apiPath] = apiStat[apiPath] || {
      n: 1,
      t: 0,
    ...
    if (limit.feqLimit) {
      ...
      const it = apiStat[apiPath]
      it.n++
      const diff = start - it.t
      if (1 / limit.feqLimit.timePerSecond * 1000 > diff) {
        ctx.body = {
          code: limit.feqLimit.code || 402,
          msg: limit.feqLimit.msg || '访问过快请稍后再试！',
          data: diff
        }
    ...
    if(limit.singleLock){
      ...
      if (it.lockKey && !(it.lockExpire && it.lockExpire <= now)) { // 当锁存在，且未过期时，不能再加锁
        ctx.body = {
          code: limit.singleLock.code || 402,
          msg: limit.singleLock.msg || '访问过快请稍后再试！' + Date.now()
        }
        routerLog(ctx.body, name || desc, apiPath, Date.now() - start)
        return ctx.body
      }
      ...
      // 加锁
      it.lockKey = lockKey = Symbol()
      it.lockExpire = now + (limit.singleLock.expire || 5000)
    }
    ...
    // 解锁
    if(limit.singleLock && lockKey && apiStat[apiPath].lockKey === lockKey){
      apiStat[apiPath].lockKey = null
      apiStat[apiPath].lockExpire = 0
    }
