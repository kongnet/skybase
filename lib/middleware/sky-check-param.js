...
const apiCheckLog = function (url, obj, method, oldParam, ctx) {
  if (config.logger) {
    ...
    const fname =
      methodStr === 'GET' ? 'dimg' : methodStr === 'POST' ? 'dimy' : 'r'
    $.log(
      $.c[fname](`[ ${methodStr} ]`) +
...
const preCheck = (ctx, method, url, apiPath) => {
  if (!['get', 'post', 'all'].includes(method)) {
    ctx.throwCode(ctx.status, 'base:only get/post/all method') //'只支持get/post/all方式'
  ...
  if (!apiSetting) {
    ctx.throwCode(404, 'base:interface does not exist') //'接口不存在'
    return { succ: false, continue: false }
  }
  ...
  if (method !== needMethod && needMethod !== 'all') {
    ...
    ctx.throwCode(405, 'base:method type error') //'方法类型不对'
    return { succ: false, continue: false }
  }
...
module.exports = async (ctx, next) => {
  ...
  const {
    ...
    needMethod,
    apiSetting
  } = preCheck(ctx, method, url, apiPath)
  if (!canContinue) {
    ...
    /*  有errSimple 看是否配置了语言参数_lang=en 或者 默认default,或者直接返回errSimple.
        没有 errSimple 返回 ctx.checkedData.msg
         errSimple: {
          ...
          errSimple:'err msg'
      */
    return ctx.throwCode(
      400,
      apiSetting.errSimple
        ? apiSetting.errSimple[params['_lang'] ? params['_lang'] : 'default'] ||
            (typeof apiSetting.errSimple === 'string'
              ? apiSetting.errSimple
              : ctx.checkedData.msg)
        : ctx.checkedData.msg
    )
